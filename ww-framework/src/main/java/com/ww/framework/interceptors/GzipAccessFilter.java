...
import com.ww.framework.ParameterResponseWrapper;

public class GzipAccessFilter implements Filter {
	...
	public void doFilter(ServletRequest rq, ServletResponse rp,
		...
		if (enable && isGZipEncoding(request)) {
			ParameterResponseWrapper zipResponse = new ParameterResponseWrapper(response);
